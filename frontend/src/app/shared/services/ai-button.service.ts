...
export class AiButtonService {

  public readonly AIPrompts = {
    businessPlanDetails: 
    ...
    portfolio:
    `Now lets estimate and consolidate typical Portfolio in json formated array of products and services provided by such kind of a business (address: ##address##  .. ##category## description: ##description## ). Your answer will be processed using javascript framework, so give the most precise answers you can.
    ...
    name - is typical most popular services provided by such business;
    cost  - cost of single service or product (if it is haircut - estimated total cost of materials for single haircut) - normally it can be obtained from typical margin of such service. Should be number; 
    price - price of single service or product. Should be number; 
    eachNCustomerBuys - Each N Customer who buys such product or service - for example haircut is normally taken by each 10th customer, but we need to estimate this number for current location and typical business. This column is hard to estimate, but please give try. Should be number;
    monthlyConversionUserThatBuys - Monthly % Conversion Customer That Buys - calculated = 1/<Each N Customer who buys> 
    ordersMonthPerCustomer - Units of Products or services per customer - is an average number of products in check, suggesting that some customers might buy 2 or more products in a month. Should be number.
    estimatedFailedOrders - The percentage of orders that are not fulfilled due to various reasons such as the customer changing their mind, the product being out of stock, or the customer not being able to pay for the product.
    onFirstInitialStock - If you are selling a physical product, then you will need to buy the product from a supplier and store it in a warehouse. Represent it's value as true or false
    beginningMonths - The number of months that you will need to pay for the initial stock. For example, if you need to pay for the initial stock in 3 months, then the number of months that you will need to pay for the initial stock is 3. Should be number
    numberOfMonthsForInitialStock - How many monthes you need to gain first sell. Should be number. Remember your ansver should be strictly of json format`,
    seasonalityByBrand:
