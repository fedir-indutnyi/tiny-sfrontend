...
import {BusinessPlanItemActions, OtherOperatingIncomeLossActions, Selectors} from '@businessplan-item/store/index';
import {iOtherOperatingIncomeLoss} from "@businessplan-item/store/reducers/other-operating-income-loss.reducer";


@Component({
    selector: 'app-other-operating-income-loss',
    templateUrl: './other-operating-income-loss.html',
    styleUrls: ['./other-operating-income-loss.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class OtherOperatingIncomeLoss implements OnInit {
    protected dataSource$!: Observable<iDynamicTableDataSource[]>;
    componentName: string = 'app-other-operating-income-loss';

    constructor(private _store: Store) {
    };

    ngOnInit(): void {
        this._store.dispatch(OtherOperatingIncomeLossActions.init());

        this.dataSource$ = this._store.select(Selectors.selectOtherOperatingIncomeLoss).pipe(
    ...
    onApply(data: iDynamicTableDataSource[]): void {
        ...
        this._store.dispatch(OtherOperatingIncomeLossActions.updateAll({payload: {otherOperatingIncomeLoss: data as unknown as iOtherOperatingIncomeLoss[]}}));
